...
#cv.seasonal
```{r, echo=TRUE}
cv.seasonal = function(ts, d.max, span.min=0.01, span.max, span.control=0.01, fold=5) {
  start_year = tsp(ts)[1]
  end_year = tsp(ts)[2]
...
```{r, echo=TRUE}
cv.trend = function(ts, d.max, span.min=0.01, span.max, span.control=0.01, fold=5) {
  start_year = tsp(ts)[1]
  end_year = tsp(ts)[2]
  k = floor(end_year-start_year)+1 #k = 38 years
  t = ceiling(k/fold) #t = t years/unit
