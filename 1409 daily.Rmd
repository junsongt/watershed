...
seasonality4 = function(ts, window, degree) {
  ...
  for (i in (1:freq)) {
    ...
    for (w in interval) {
      ...
      # # set quantile & subset cluster without outlier(usually this step takes time)
      # range = quantile(cluster, prob = c(0.25, 0.75))
      # lb = as.numeric(range[1])
      # ub = as.numeric(range[2])
      # cluster = cluster[lb<=cluster&cluster<=ub]
...
lines(s4, type = "l", xlim = c(0,1000), col = "green")
```
  ...
  for (w in (0:window.max)) {
    for (d in (0:3)) {
      ...
      for (year in (1:k)) {
        ...
        seasonality = seasonality3(train, w, d)
