...
plot(ts1409, type="l", xlim = c(2013,2018),col="green", ylim=c(-5,5))
lines(t1, col="red")
lines(t2, col="blue")
...
convergence.test = function(ts, iter) {
  ...
  for (i in (1:iter)) {
    if (i == 1) {
      ...
      curr_trend = trend.loess(ts - curr_season, span=1/30)
      ...
      curr_trend = trend.loess(ts - prev_season, span=1/30)
      dt = c(dt, mean((curr_trend - prev_trend)^2, na.rm=TRUE))
      prev_trend = curr_trend
    }
    else {
