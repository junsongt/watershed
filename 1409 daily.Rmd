...
```{r, echo=TRUE}
convergence.test(ts1409, iter=5)
...
```{r, echo=TRUE}
t = trend.ma(ts1409)
s = seasonal.loess(ts1409 - t, deg=1, span = 1/38)
```
...
legend("topright",legend=c("ts","loess","loess by MA", "stl_plus"), lty=1,
       col=c("green","red","orange", "blue"))


# seasonal compare
plot(ts1409, type="l", col="green", xlim=c(1980,1982), ylim=c(-5,5), main="1409 seasonal compare")
...
# remainder compare
plot(ts1409, type="l", col="green", ylim=c(-5,5), main="1409 remainder compare")
...
acf(decomp1409$remainder, lag.max = 365*3, main = "acf of daily response", na.action = na.pass)
...
#short term trend
```{r, echo=TRUE}
short_trend = trend.loess(decomp1409$remainder, deg=2, span=90/length(ts1409))
...
acf(error, lag.max = 365,  na.action = na.pass)
pacf(error, lag.max = 50,  na.action = na.pass)
...
# model fitting
```{r, echo=TRUE}
model = arima(ts(stlp1409$data[,"remainder"], start =c(1980,1), frequency = 365), order = c(5,0,0), include.mean = TRUE)
acf(model$residuals, na.action = na.pass)
pacf(model$residuals, lag.max=365*5, na.action = na.pass)
tsdiag(model)
