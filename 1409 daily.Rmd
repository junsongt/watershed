...
# 2nd version
# (1) x = (-2,-1,0,1,2,-2,-1,0,1,2,...); y = ($,$,$,$,$,&,&,&,&,&,...) horizontal then vertical
```{r, echo=TRUE}
seasonality1 = function(ts, window, degree) {
  ...
  for (i in (1:freq)) {
    ...
    for (j in (1:k)) {
      ...
      ub = min(center+window, k*freq)
      idx = seq(from=lb,to=ub,by=1)
      normalize_idx = idx - center
      x = c(x, normalize_idx)
      # NA for y at idx does not matter
      y = c(y, ts[idx])
    ...
    if (degree == 0) {
      model = lm(y ~ 1)
      yhat = as.numeric(model$fitted.values[which(x==0)])
    }
    if (degree == 1) {
      model = lm(y ~ x)
      yhat = as.numeric(model$fitted.values[which(x==0)])
