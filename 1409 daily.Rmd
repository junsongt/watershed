...
```{r, echo=TRUE}
start_time = Sys.time()
m = same_day_cluster(ts1409, trim=0)
end_time = Sys.time()
end_time - start_time
...
# long runtime!!!
start_time = Sys.time()
bestSeason(ts1409, window.max = 11, trim_per.max = 0.25)
end_time = Sys.time()
end_time - start_time
```
...
```{r, echo=TRUE}
start_time = Sys.time()
bestSeason(ts1409, window.max = 10, trim_per.max = 0)
end_time = Sys.time()
end_time - start_time
...
```{r, echo=TRUE}
start_time = Sys.time()
best_seasonal = seasonality(ts1409, window=11, degree=0, trim=0.05)
end_time = Sys.time()
end_time - start_time
plot(ts1409, col = "green", xlim=c(1980,2000), ylim=c(-5,5), type="l", main="best seasonality(11, 0, 5%) & raw ts")
lines(ts(best_seasonal, start=c(1980,1), frequency=365),col = "blue")
...
lines(ts(t1, start = c(1980,1), frequency = 365), col = "blue")
...
# remove seasonality ---> remove trend
```{r, echo=TRUE}
ts_ds = ts1409 - s1
r1 = ts_ds - trend(ts_ds)
plot(r1, ylim = c(-5,5))
qqnorm(r1)
qqline(r1)
...
# remove trend ---> remove seasonality
```{r, echo=TRUE}
ts_dt = ts1409 - trend(ts1409)
r2 = ts_dt - seasonality(ts_dt, window=11, degree = 0, trim=0.05)
plot(r2, ylim = c(-5,5))
qqnorm(r2)
qqline(r2)
...
```{r, echo=TRUE}
seasonal = function(ts, deg=2, span=0.75, iter=4) {
  k = floor(tsp(ts)[2]-tsp(ts)[1])+1
  freq = as.integer(tsp(ts)[3])
  # form data frame
  x = NULL
...
```{r, echo=TRUE}
start_time = Sys.time()
s3 = seasonal(ts1409, span=1/30)
end_time = Sys.time()
end_time - start_time
plot(df$y ~ df$x, col = "green", main = "one full period")
lines(best_seasonal[1:365], col = "red")
...
lines(ts(s3, start=c(1980,1), frequency = 365), col="blue")
lines(ts(best_seasonal, start=c(1980,1), frequency = 365), col="red")
```

#CV
```{r, echo=TRUE}
best_season = function(ts, d.max, s.max) {
  start_year = tsp(ts)[1]
  end_year = tsp(ts)[2]
  k = floor(tsp(ts)[2]-tsp(ts)[1])+1 #k = 38 years
  t = ceiling(k/5) #t = 8 years/unit
  ...
  for (d in (0:d.max)) {
    for (s in (1:(s.max/0.05))) {
      ...
      for (i in (1:5)) {
        ...
        train = ts[-test_index]
        train = ts(train, start=c(start_year, 1), frequency = freq)
        seasonality = seasonal(train, deg=d, span=s*0.05)
...
```{r, echo=TRUE}
start_time = Sys.time()
best_season(ts1409, d.max=2, s.max=0.5)
end_time = Sys.time()
end_time - start_time
...
trend.loess = function(ts, deg=2, span=0.75, iter=4) {
  trend = NULL
  # 1st cursor, controling pos in whole ts
  start_pos = 1
  while (start_pos <= length(ts)) {
    x = NULL
    y = NULL
    # 2nd cursor, controling accumulating consecutive non-NA
    i = start_pos
    while (!is.na(ts[i])) {
      y = c(y, ts[i])
      x = c(x, i)
...
start_time = Sys.time()
t2 = trend.loess(ts1409, span=1/6)
end_time = Sys.time()
end_time - start_time

ts1409_dt = ts1409 - ts(t2, start=c(1980,1), frequency = 365)
s4 = ts(seasonal(ts1409_dt, span=1/6), start=c(1980,1), frequency = 365)

plot(ts1409, type="l", col="green", ylim=c(-5,5))
# lines(ts(t1, start = c(1980,1), frequency = 365), col="red")
# lines(ts(t2, start = c(1980,1), frequency = 365), col="blue")
lines(s4, col = "red")
lines(ts(best_seasonal, start=c(1980,1), frequency = 365), col="blue")
