---
title: "1409 daily"
author: "Junsong Tang"
date: "5/16/2022"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
library(knitr)
library(tseries)
library(xts)
library(zoo)
library(tidyverse)
library(forecast)
library(stlplus)
```


# watershed 1409 has snow fraction 3%
=======================================================================================
# Daily unimputed data
```{r, echo=TRUE}
data1409 = DATA[[1409]]
daily1409 = as.daily(data1409)

# identifying the range of NA
daily1409[is.na(daily1409$resp),]

# subset of log-daily from 1980.1 - 1994.12(15 years)
ts1409 = ts(log(daily1409$resp), start=c(1980,1), frequency = 365)
plot(ts1409,ylim=c(-5,5))
acf(ts1409, lag.max = 365*5, main = "acf of daily response", na.action = na.pass)
pacf(ts1409, lag.max = 365*5, main = "pacf of daily respoonse", na.action = na.pass)

# # train set 12 years, test set 3 years
# daily_train = window(daily_ts, start=c(1980,1), end=c(1991,365))
# daily_test = window(daily_ts, start=c(1992,1), end=c(1994,365))


# plot(daily_train, col="red", xlim = c(1980, 1995), main="Train & Test")
# lines(daily_test, col="blue")
```


# p = TSA::periodogram(window(ts1409, start=c(1980, 1), end=c(1996,12)), xlim = c(0,0.01))
# d = data.frame(freq=p$freq, spec=p$spec)
# order = d[order(-d$spec),]
# top5 = head(order, 5)
# 1/top5$freq

```{r, echo=TRUE}
spec1409 = spectrum(ts1409, na.action=na.exclude, xlim = c(0,0.02), ylim=c(10, 10000), type = "h")
spec_chart1409 = data.frame(period=1/spec1409$freq, frequency=spec1409$freq, spectrum=spec1409$spec)
head(spec_chart1409[order(spec_chart1409$spectrum, decreasing=TRUE),], 10)

```










# CV on seasonal
```{r, echo=TRUE}
start_time = Sys.time()
cv.seasonal(ts1409, d.max=2, fold=5)
end_time = Sys.time()
end_time - start_time

```
Comment:
$$span \in [\frac{10}{\sqrt{n}}, \frac{20}{\sqrt{n}}] \Rightarrow span = \frac{15}{\sqrt{n}}]$$



# CV on trend
```{r, echo=TRUE, warning=FALSE}
set.seed(2022)
start_time = Sys.time()
cv.trend(ts1409 - seasonal.loess(ts1409, deg=1, span=15/sqrt(length(ts1409))), d.max=2, fold=5)
end_time = Sys.time()
end_time - start_time

```
Comment: 
$$span \in [\frac{0.5}{\sqrt{n}}, \frac{1}{\sqrt{n}}] \Rightarrow span = \frac{0.5}{\sqrt{n}}$$

Comment:
$$span \in [\frac{0.1}{\sqrt{n}}, \frac{1}{\sqrt{n}}] \Rightarrow span = \frac{0.1}{\sqrt{n}}$$

Comment: the smaller the span on trend, the higher the degree, the better



```{r, echo=TRUE}
convergence.test(ts1409, iter=7)

```









#stl plus
```{r, echo=TRUE}
stlp1409 = stlplus(ts1409, n.p=365, s.window="periodic", inner=2, outer=10)
# my_s = seasonal.loess(ts1409, deg=1, span=0.1273659)
# my_t = trend.loess(ts1409-my_s, deg=1, span=0.0008491057)
plot(stlp1409)

```

```{r, echo=TRUE, warning=FALSE}
# no cv
start_time = Sys.time()
obj1= decompose.loess(ts1409)
end_time = Sys.time()
end_time - start_time
```



```{r, echo=TRUE, warning=FALSE}
# cv on seasonal in iterations
start_time = Sys.time()
obj2= decompose.loess(ts1409)
end_time = Sys.time()
end_time - start_time
```

```{r, echo=TRUE}
plot(obj1$seasonal, xlim=c(1980,1982), col="red", main="seasonal")
lines(obj2$seasonal, col="blue")
legend("topright",legend=c("no cv","cv"), lty=1,
       col=c("red", "blue"))

plot(obj1$trend, ylim=c(-3,3), col="red",main="trend")
lines(obj2$trend, col="blue")
legend("topright",legend=c("no cv","cv"), lty=1,
       col=c("red", "blue"))

plot(obj1$short_trend, ylim=c(-3,3), col="red", main="short term trend")
lines(obj2$short_trend, col="blue")
legend("topright",legend=c("no cv","cv"), lty=1,
       col=c("red", "blue"))

plot(obj1$remainder, ylim=c(-3,3), col="red", main="remainder")
lines(obj2$remainder, col="blue")
legend("topright",legend=c("no cv","cv"), lty=1,
       col=c("red", "blue"))
```



```{r, echo=TRUE, warning=FALSE}
start_time = Sys.time()
decomp1409 = decompose.loess(ts1409)
end_time = Sys.time()
end_time - start_time
```



```{r, echo=TRUE}
# trend compare
plot(ts1409, type="l", col="green", ylim=c(-5,5), main="1409 trend compare")
lines(decomp1409$trend, col="red")
lines(ts(stlp1409$data[,"trend"], start =c(1980,1), frequency = 365), col="blue")
legend("topright",legend=c("ts","loess", "stl_plus"), lty=1,
       col=c("green","red", "blue"))


# seasonal compare
plot(ts1409, type="l", col="green", xlim=c(1980,1982), main="1409 seasonal compare")
lines(decomp1409$seasonal, col="red")
lines(ts(stlp1409$data[,"seasonal"], start =c(1980,1), frequency = 365), col="blue")
legend("topright",legend=c("ts","loess", "stl_plus"), lty=1,
       col=c("green","red", "blue"))


# remainder compare
plot(ts1409, type="l", col="green", xlim=c(1980,1985), ylim=c(-5,5), main="1409 remainder compare")
lines(decomp1409$remainder, col="red")
lines(ts(stlp1409$data[,"remainder"], start =c(1980,1), frequency = 365), col="blue")
legend("topright",legend=c("ts","loess", "stl_plus"), lty=1,
       col=c("green","red", "blue"))

```


```{r, echo=TRUE}
mean((ts1409 - decomp1409$trend)^2, na.rm=TRUE)
mean((ts1409 - ts(stlp1409$data[,"trend"], start =c(1980,1), frequency = 365))^2, na.rm=TRUE)

mean((ts1409 - decomp1409$seasonal)^2, na.rm=TRUE)
mean((ts1409 - ts(stlp1409$data[,"seasonal"], start =c(1980,1), frequency = 365))^2, na.rm=TRUE)


mean((decomp1409$remainder)^2, na.rm = TRUE)
mean((ts(stlp1409$data[,"remainder"], start =c(1980,1), frequency = 365))^2, na.rm=TRUE)

```





```{r, echo=TRUE}
# sub_season = subseasonal.loess(decomp1409$remainder, freq = 365/2, deg=1, span=15/sqrt(length(ts1409)))
plot(ts1409, type="l", col="green", ylim=c(-5,5))
lines(seasonal.loess(decomp1409$remainder, deg=1, span=15/sqrt(length(ts1409))), col="red")

acf(decomp1409$remainder, lag.max = 50, main = "acf of daily response", na.action = na.pass)
pacf(decomp1409$remainder, lag.max = 50, main = "pacf of daily respoonse", na.action = na.pass)


```



```{r, echo=TRUE}
acf(ts(stlp1409$data[,"remainder"], start =c(1980,1), frequency = 365), lag.max = 365,  na.action = na.pass)
pacf(ts(stlp1409$data[,"remainder"], start =c(1980,1), frequency = 365), lag.max = 50,  na.action = na.pass)

```



#short term trend
```{r, echo=TRUE}
# short_trend = trend.loess(decomp1409$remainder, deg=2, span=7/length(ts1409))
# error = decomp1409$remainder - short_trend
# 
# plot(ts1409, xlim=c(1980,1982),type="l", col="green", ylim=c(-5,5))
# lines(decomp1409$trend, col="red")
# lines(short_trend, col="blue")

plot(ts1409, xlim=c(1980,1985), type="l", col="green", ylim=c(-5,5))
lines(decomp1409$trend, col="red")
lines(decomp1409$short_trend, col="orange")
lines(decomp1409$remainder, col="blue")

acf(decomp1409$remainder, lag.max = 50,  na.action = na.pass)
pacf(decomp1409$remainder, lag.max = 50,  na.action = na.pass)



```





# model fitting
```{r, echo=TRUE}
model = arima(error, order = c(3,0,0), include.mean = TRUE)
acf(model$residuals, na.action = na.pass)
pacf(model$residuals, na.action = na.pass)
tsdiag(model)
plot(model$residuals, main="model residuals")
qqnorm(model$residuals)
qqline(model$residuals)
hist(model$residuals)

```




<!-- ```{r, echo=TRUE} -->
<!-- fivedaily_df = as.gasp.data(data1409, window=5) -->
<!-- save(fivedaily_df, file="1409 5-day avg.RData") -->
<!-- ``` -->









