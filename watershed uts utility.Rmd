...
seasonal = function(ts, deg=2, window=length(ts), iter=4) {
  ...
  y = NULL
  z = NULL
  # bumper is number of traceback days from head or traceforward days from tail
  bumper = ceiling(window / k)
  
  for (i in ((1-bumper):(freq+bumper))) {
    if (i < 1) {
  ...
  full_period = loess_obj$fitted[which(z==TRUE)][c((bumper+1) : (bumper+freq))]
  seasonality = rep(full_period, k)
