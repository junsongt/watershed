...
#     if (i == 1) {
#       curr_season = seasonal.loess(ts, deg=s.deg, span=s_span)
#       curr_trend = trend.loess(ts - curr_season, deg=t.deg, span=t_span)
#       prev_season = curr_season
#       prev_trend = curr_trend
#     }
#     else if (i %% 2 != 0) {
#       curr_trend = trend.loess(ts - prev_season, deg=t.deg, span=t_span)
#       prev_trend = curr_trend
#     }
#     else {
#       curr_season = seasonal.loess(ts - prev_trend, deg=s.deg, span=s_span)
#       prev_season = curr_season
#     }
#   }
#   
...
decompose.loess = function(ts, s.deg=1, t.deg=1, s.window=NULL, t.window=NULL, iter=3) {
  ...
  for (i in (1:true_iter)) {
    if (i == 1) {
      curr_trend = trend.loess(ts, deg=t.deg, span=t_span)
      curr_season = seasonal.loess(ts - curr_trend, deg=s.deg, span=s_span)
      prev_trend = curr_trend
      prev_season = curr_season
    }
    else if (i %% 2 == 0) {
...
```{r, echo=TRUE}
subseasonal.loess = function(ts, freq, deg=2, span=0.75, iter=4) {
  k = floor(tsp(ts)[2]-tsp(ts)[1])+1
  main_freq = as.integer(tsp(ts)[3])
  start_year = as.integer(tsp(ts)[1])
  # form data frame arranged in same day order
  x = NULL
  y = NULL
  ...
  return (ts(seasonality, start=c(start_year,1), frequency = main_freq))
}
