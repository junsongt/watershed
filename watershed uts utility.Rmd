...
```{r, echo=TRUE}
start_time = Sys.time()
fiveday = as.gasp.data(data1409, 5)
end_time = Sys.time()
end_time - start_time
...
decompose.loess = function(ts, s.deg=1, t.deg=1, s.window=NULL, t.window=NULL, iter=3, cv.control = list(increment=NULL, fold=5)) {
  freq = as.integer(tsp(ts)[3])
  n = length(ts)
  k = ceiling(n/freq)
  true_iter = 2 * iter - 1
  ...
  for (i in (1:true_iter)) {
    if (i == 1) {
      ...
      opt_deg = opt_para$best_deg
      opt_window = opt_para$best_window
      curr_season = seasonal.loess(ts - prev_trend, deg=opt_deg, window=opt_window)
      prev_season = curr_season
    }
  }
...
decompose.loess.plus = function(ts, s.deg=1, t.deg=1, st.deg=2, s.window=NULL, t.window=NULL, st.window=NULL, iter=3, cv.control = list(increment=NULL, fold=5)) {
  freq = as.integer(tsp(ts)[3])
  n = length(ts)
  k = ceiling(n/freq)
  true_iter = 3 * iter - 2
