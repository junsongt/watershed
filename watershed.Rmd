...
plot.shape = function(shapes, content=NULL, map=TRUE, option) {
  ...
  if (map) {
    ...
    if (option %in% c("rain", "snow", "temp", "gauge")) {
      pt = data.frame(center_longs, center_lats)
      # pt = st_as_sf(pt, coords = c("center_longs", "center_lats"))
      m = addMarkers(m, lng=center_longs, lat=center_lats, group="pt")
...
plot.watersheds = function(watersheds, map = TRUE, option) {
...
plot.watersheds(watersheds=load.watersheds(c(1,1013,1409)), map=TRUE, option="gauge")
plot.watersheds(watersheds=load.watersheds(c(1, 77, 101, 31)), map=TRUE, option="fraction")
