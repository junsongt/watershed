...
# gridcode
# shape object(could be null when initializing)
...
# shape has to be "SpatVector" class object(type is S4), non-null
```{r, echo=TRUE}
Watershed = function(data, gridcode, shape) {
  # check shape & gridcode
  # (1) all inputs are correct
  if (class(shape) == "SpatVector" && shape$gridcode == gridcode &&
      is.numeric(gridcode) && gridcode %in% gridCodes){
    obj = NULL
    if (is.null(data)) {
      mts = NULL
    }
  ...
  else if (!gridcode %in% gridCodes) {
    stop("such grid code does not exist!")
    return ()
  }
  else {
    stop("shape is not SpatVector object!")
    return ()
  }
...
```{r, echo=TRUE}
start_time = Sys.time()
ws1409 = Watershed(data1409, 1409, shapes[1368])
end_time = Sys.time()
end_time-start_time
...
load_watersheds = function(gridcodes) {
  ...
  for (i in (1:length(gridcodes))) {
    ...
    if (!(filename %in% fileNames)) {
      ...
      data = mget(load(paste0(path, gc, "_dat.RData")))
    }
    shape = shapes[which(shapes$gridcode == gc)]
    ws = Watershed(data, gc, shape)
    watersheds[[i]] = ws
  }
  return (watersheds)
...
```{r, echo=TRUE}
start_time = Sys.time()
# load_watersheds(c(1003, 1013, 1409))
ws_1_2_1409 = load_watersheds(c(1, 2, 1409))
end_time = Sys.time()
end_time-start_time
```
