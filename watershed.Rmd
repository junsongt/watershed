...
for (i in (1:length(shapes))) {
  shapeIDs = c(shapeIDs, shapes[i]$gridcode)
}
...
cube_to_mts = function(Rdata) {
  df = data.frame()
  for (i in (1:(length(Rdata)-1))) {
    mtx = Rdata[[i]]
    start = Rdata$yrs[1]
    end = Rdata$yrs[length(Rdata$yrs)]
    ts = matrix_to_ts(mtx, start_year=start, end_year=end)
    if (dim(df)[1] == 0) {
      df = data.frame(ts)
...
# snow fraction(faster version)
# input: Rdata file
# return: (double) snow fraction
snow_fraction = function(Rdata) {
  snow = Rdata$snow_mat
  rain = Rdata$rain_mat
  snow_sum = 0
  rain_sum = 0
  ...
  for (r in (1:nrow(snow))) {
    for (c in (1:length(snow[1,]))) {
      # NA issue(if one of them missing, ignore both)
...
# rain fraction(faster version)
rain_fraction = function(Rdata) {
  return (1 - snow_fraction(Rdata))
}
...
<!-- # snow fraction & rain fraction on mts object -->
<!-- ```{r, echo=TRUE} -->
...
<!--   # assume that snow & rain matrices have same lengths -->
<!--   for (i in (1:length(snow))) { -->
<!--     # NA issue -->
<!--     if (!is.na(snow[i]) && !is.na(rain[i])) { -->
<!--       snow_sum = snow_sum + snow[i] -->
<!--       rain_sum = rain_sum + rain[i] -->
<!--     } -->
<!--   } -->
...
# test time for snow fraction
```{r, echo=TRUE}
start_time = Sys.time()
snow_fraction(data1409)
end_time = Sys.time()
end_time - start_time
...
```{r, echo=TRUE}
# input shape files, 
# return grid codes(watershed No.) + snow fractions + rain fractions in data frame
...
render_piechart = function(shapes) {
  df = get_fraction(shapes)
  for (i in (1:(dim(df)[1]))) {
    row = as.vector(as.numeric(df[i,]))
    pie(row[-1], labels = c("snow fraction", "rain fraction"),
        main = paste("watershed", as.character(row[1])))
  }
}


get_ts_summary = function(shapes) {
...
```{r, echo}
load_content = function(shapes, option) {
  content = NULL
  for (i in (1:length(shapes))) {
    switch(option,
      "fraction" = {
        stats = get_fraction(shapes)
        sr = stats[i,][2]
        rr = stats[i,][3]
...
plot_shape = function(shapes, map=TRUE, option="fraction") {
  # center_longs = shapes$Longitude
  # center_lats = shapes$Latitude
  stats = get_fraction(shapes)
  content = load_content(shapes, option = option)
...
plot_shape(shapes[1:4], TRUE, "fraction")
# plot_shape(shapes[c(1, 11, 101, 31, 77)], TRUE, "fraction")
plot_shape(shapes[c(1061,1273,1368)], TRUE, "fraction")
# plot_shape(shapes[c(1061,1273,1368)], FALSE, "fraction")
```
...
```{r, echo=TRUE}
# rsf is data.frame
# values is vector of new values
accumulate_frame = function(rsf, values) {
